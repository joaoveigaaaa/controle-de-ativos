...
def register_inventory(app, configuration, connect, env):
    ...
    def schema(cursor, table, serial):
        ...
        aliases = {'ativo': ['ativo', 'nome', 'equipamento', 'descricao', 'produto'],
                   ...
                   'data_garantia': ['data_garantia', 'garantia'], 'departamento': ['departamento', 'dp']}
    ...
    def check_connection():
        with database() as (_, cursor, table, serial):
            ...
            if 'ativo' not in mapping:
                raise ValueError('A conexão funcionou, mas falta mapear a coluna do nome do ativo em MYSQL_FIELD_MAP.')
            return jsonify(ok=True, mensagem='Conexão com MySQL e colunas principais verificadas. Nenhum registro foi alterado.', tabela=table, coluna_serie=serial)
    ...
    def spreadsheet():
        ...
        try:
            ...
            synonyms = {
                'ativo': ['Nome Ativo', 'Equipamento', 'Descrição', 'Item'],
                'numero_serie': ['Série', 'Serial', 'Número', 'Nº de Série', 'N° Série', 'N/S', 'S/N', 'Serial Number'],
                'categoria': ['Tipo'], 'status': ['Estado', 'Situação'],
                'matricula': ['RE', 'ID Colaborador'],
                'data_garantia': ['Data Garantia', 'Garantia'],
                'departamento': ['DP', 'Setor', 'Área'],
                'responsavel': ['Nome Responsável', 'Colaborador', 'Nome Colaborador', 'Usuário'],
                'sede': ['Empresa', 'Unidade', 'Filial', 'Localidade', 'Sede/Empresa'],
            }
            for field, labels in synonyms.items():
                aliases.update({norm(label): field for label in labels})
            iterator = sheet.iter_rows()
            required = {'ativo', 'numero_serie'}
            best = set()
            sales = False
            # Titles and blank lines may precede the real column headings.
            for header_number, cells in enumerate(iterator, 1):
                headers = [aliases.get(norm(c.value)) for c in cells]
                known = set(headers) - {None}
                labels = {norm(c.value) for c in cells}
                sales = sales or {'cliente', 'valor'}.issubset(labels)
                if len(known & required) > len(best & required):
                    best = known
            ...
            if not required.issubset(headers):
                if sales and not best:
                    raise ValueError('Esta planilha é de vendas (Cliente e Valor). Para importar equipamentos, selecione uma planilha com Ativo e Número de série. Use Baixar modelo Excel para preencher os dados.')
                missing = [label for field, label in zip(FIELDS, LABELS) if field in required - best]
                raise ValueError('Não encontrei as colunas obrigatórias: ' + ', '.join(missing) + '. Coloque os cabeçalhos na mesma linha, entre as primeiras 30 linhas da primeira aba. Use Baixar modelo Excel como referência.')
            known = [h for h in headers if h]
            if len(known) != len(set(known)):
                duplicates = [label for field, label in zip(FIELDS, LABELS) if known.count(field) > 1]
                raise ValueError(f'Linha {header_number}: há mais de uma coluna para ' + ', '.join(duplicates) + '. Mantenha uma coluna para cada campo.')
            records = []
