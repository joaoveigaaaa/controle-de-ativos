...
def create_app(settings=None, connector=None):
    ...
    env.setdefault('MYSQL_DATABASE', 'numero_de_serie')
    env.setdefault('MYSQL_TABLE', 'ativos')
    env.setdefault('MYSQL_SERIAL_COLUMN', 'numero')
...
if __name__ == '__main__':
    ...
    if '--verificar' in sys.argv:
        with create_app().test_client() as client:
            result = client.get('/api/conexao')
            data = result.get_json()
            print(data.get('mensagem') or data.get('erro'), flush=True)
            if result.status_code == 200:
                print(f'Tabela: {data["tabela"]} | Coluna da série: {data["coluna_serie"]}', flush=True)
        raise SystemExit(0 if result.status_code == 200 else 1)
